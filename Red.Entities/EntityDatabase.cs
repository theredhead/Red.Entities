...
namespace Red.Entities
{
	public class EntityDatabase
	{
		...
		internal void Insert(Entity entity)
		{
			...
			{
				command.CommandText = StatementBuilder.BuildInsertCommandText(entity.EntityType);
				foreach (string fieldName in entity.EntityType.FieldNames)
				{
					IDbDataParameter parameter = command.CreateParameter();
					parameter.ParameterName = StatementBuilder.FormatParameterName(fieldName);
					parameter.Value = entity[fieldName];
					command.Parameters.Add(parameter);
				}
		...
		internal void Update(Entity entity)
		{
			using (IDbCommand command = Connection.CreateCommand())
			{
				command.CommandText = StatementBuilder.BuildUpdateCommandText(entity.EntityType);
				foreach (string fieldName in entity.EntityType.FieldNames)
				{
					IDbDataParameter parameter = command.CreateParameter();
					parameter.ParameterName = StatementBuilder.FormatParameterName(fieldName);
