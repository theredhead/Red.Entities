            ...
            foreach (var condition in predicate.Conditions)
            {
                ...
                switch (condition.Condition)
                {
                    ...
                    case ComparisonCondition.Contains:
                        stringConditions.Add($"CHARINDEX([{condition.FieldName}], {FormatParameterName(condition.ParameterName)}) > 0");
                        break;
                    ...
                    case ComparisonCondition.BeginsWith:
                        stringConditions.Add($"CHARINDEX([{condition.FieldName}], {FormatParameterName(condition.ParameterName)}) = 1");
                        break;
                    case ComparisonCondition.EndsWith:
                        stringConditions.Add($"CHARINDEX([{condition.FieldName}], {FormatParameterName(condition.ParameterName)}) = LENGTH({condition.FieldName}-LENGTH({FormatParameterName(condition.ParameterName)}))");
                        break;
